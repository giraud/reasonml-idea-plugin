...
public class ORReferenceAnalyzer {
    ...
    static @NotNull Deque<PsiElement> createInstructionsBackward(@NotNull PsiElement sourceElement, boolean isLower, @NotNull ORLangTypes types) {
        ...
        } else if (itemType == types.C_MODULE_BINDING) {
            PsiElement sourceParent = item.getParent();
            if (sourceParent instanceof RPsiInnerModule gpModule && gpModule.getAlias() != null) {
                // source element is from a module alias, skip the module
                item = ORUtil.prevSibling(sourceParent);
                item = item == null ? sourceParent.getParent() : item;
            }
        }
            ...
            if (!inPath) {
                ...
                //}
                    ...
                    while (nextItemType == types.AND) {
                        ...
                        if (nextElement instanceof RPsiInnerModule) {
                    ...
                    while (nextItemType == types.AND) {
                        PsiElement nextElement = ORUtil.nextSibling(nextItem);
                        if (nextElement instanceof RPsiLet) {
                            instructions.push(nextElement);
                            nextItem = PsiTreeUtil.nextCodeLeaf(nextElement);
                            nextItemType = nextItem != null ? nextItem.getNode().getElementType() : null;
                        } else {
    ...
    static @NotNull List<RPsiQualifiedPathElement> processInstructions(@NotNull Deque<PsiElement> instructions, @NotNull List<ResolutionElement> resolutions, @NotNull FileBase sourceFile, @NotNull PsiManager psiManager, @NotNull Project project, @NotNull GlobalSearchScope scope) {
        ...
        while (!instructions.isEmpty()) {
            ...
            switch (instruction) {
                ...
                // identifier
                // ----------
                case RPsiLowerSymbol foundLower -> {
...
//                    for (int i = resolutions.size() - 1; i >= 0; i--) {
                ...
                // X.Y  (module path)
                // ------------------
                case RPsiUpperSymbol foundUpper -> {
                ...
                // START TAG
                // ---------
                case RPsiTagStart rPsiTagStart -> {
                ...
                case RPsiTagProperty foundProperty -> {
                    if (LOG.isTraceEnabled()) {
    ...
    static class FirstInPath extends ORFakeResolvedElement {
        ...
        public @NotNull String toString() {
            return "FirstInPath: " + getOriginalElement().getText();
        }
    }
