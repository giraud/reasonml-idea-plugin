...
public class SignatureParsingTest extends NsParsingTestCase {
  ...
  public void test_optionalFun() {
    PsiLet let =
        first(letExpressions(parseCode("let x:int => option<string> => string = (a,b) => c")));
  ...
  public void test_optionalFunParameters() {
    PsiLet let =
        first(
            letExpressions(
                parseCode("let x = (a:int, b:option<string>, c:bool=false, d:float=?) => 3")));
  ...
  public void test_externalFun() {
    PsiExternal e =
        first(
            externalExpressions(
                parseCode("external refToJsObj : reactRef => {..} = \"%identity\"")));
  ...
  public void test_externalFun2() {
    ...
    ORSignature signature = e.getORSignature();
    Collection<PsiSignatureItem> signatureItems =
        PsiTreeUtil.findChildrenOfType(e.getPsiSignature(), PsiSignatureItem.class);
  ...
  public void test_option() {
    ...
    PsiSignatureItem sigItem =
        ORUtil.findImmediateChildrenOfClass(e.getPsiSignature(), PsiSignatureItem.class)
            .iterator()
            .next();
