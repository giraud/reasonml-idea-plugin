...
public class FunctionCallParsingTest extends NsParsingTestCase {
  ...
  public void test_call2() {
    PsiLet e =
        first(
            letExpressions(
                parseCode(
                    "let _ = Belt.Option.map(self.state.timerId^, Js.Global.clearInterval)")));
  ...
  public void test_unitLast() {
    ...
    PsiFunctionCallParams params = PsiTreeUtil.findChildOfType(e, PsiFunctionCallParams.class);
    assertSize(2, params.getParametersList());
    // assertEquals("()", ORUtil.findImmediateFirstChildOfType(new
    // ArrayList<>(params.getParametersList()).get(1), (IElementType) m_types.C_UNIT).getText());
  }
