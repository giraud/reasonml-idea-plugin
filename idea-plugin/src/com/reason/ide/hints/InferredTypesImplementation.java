...
public class InferredTypesImplementation implements InferredTypes {
  ...
  @NotNull
  private LogicalPositionSignature makeLogicalPositionSignature(
      @NotNull Language lang, @NotNull LogicalORSignature value) {
    return makeLogicalPositionSignature(
        value.getLogicalStart().column,
        value.getLogicalEnd().column,
        value.getSignature().asString(lang));
  }

  @NotNull
  private LogicalPositionSignature makeLogicalPositionSignature(
      int colStart, int colEnd, String signature) {
      ...
      case IDENT:
        {
          ...
          PsiFile psiFile =
              PsiFileFactory.getInstance(project)
                  .createFileFromText("Dummy", OclLanguage.INSTANCE, "let x:" + tokens[2]);
          PsiSignature parsedSignature = PsiTreeUtil.findChildOfType(psiFile, PsiSignature.class);
          if (parsedSignature != null) {
            ORSignature orSignature = parsedSignature.asHMSignature();
            m_signatures.put(start, orSignature);
  ...
  private void addVisibleSignature(
      @NotNull LogicalPosition lStart,
      @NotNull LogicalPosition lEnd,
      @NotNull ORSignature signature) {
  ...
  static class OpenModule {
    @NotNull private final LogicalPosition m_position;
  ...
  static class LogicalORSignature {
    @NotNull private final LogicalPosition m_lStart;
    @NotNull private final LogicalPosition m_lEnd;
    @NotNull private final ORSignature m_signature;
