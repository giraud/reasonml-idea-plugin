...
public final class PsiFinder {
  ...
  static class PartitionedModules {
    ...
    PartitionedModules(
        ...
        @Nullable Collection<PsiModule> modules,
        @Nullable ModuleFilter<PsiModule> filter) {
      if (modules != null) {
  ...
  public Set<PsiLet> findLets(@NotNull String name, @NotNull ORFileType fileType) {
    return findLowerSymbols(
        "lets", name, fileType, IndexKeys.LETS, PsiLet.class, allScope(m_project));
  }

  @NotNull
  public Set<PsiVal> findVals(@NotNull String name, @NotNull ORFileType fileType) {
    return findLowerSymbols(
        "vals", name, fileType, IndexKeys.VALS, PsiVal.class, allScope(m_project));
  }

  @NotNull
  public Set<PsiType> findTypes(@NotNull String name, @NotNull ORFileType fileType) {
    return findLowerSymbols(
        "types", name, fileType, IndexKeys.TYPES, PsiType.class, allScope(m_project));
  }
  ...
  public Set<PsiExternal> findExternals(@NotNull String name, @NotNull ORFileType fileType) {
    return findLowerSymbols(
        "externals", name, fileType, IndexKeys.EXTERNALS, PsiExternal.class, allScope(m_project));
  }
  ...
  @Nullable
  public PsiVariantDeclaration findVariant(
    ...
    Collection<PsiVariantDeclaration> variants =
        VariantFqnIndex.getInstance().get(qname.hashCode(), m_project, scope);
    return variants.isEmpty() ? null : variants.iterator().next();
  }

  @NotNull
  public Collection<PsiVariantDeclaration> findVariantByName(
      @Nullable String path, @Nullable String name, @NotNull GlobalSearchScope scope) {
    if (name == null) {
      return Collections.emptyList();
    }

    Collection<PsiVariantDeclaration> variants =
        VariantIndex.getInstance().get(name, m_project, scope);
  ...
  @NotNull
  public Set<PsiModule> findModulesFromQn(
