...
public class ORFileEditorListener implements FileEditorManagerListener {
    ...
    if (FileHelper.isCompilable(fileType)) {
      ...
      if (selectedEditor instanceof TextEditor && document != null) {
        ...
        document.addDocumentListener(
            new ORDocumentEventListener(insightUpdateQueue), selectedEditor);

        ORPropertyChangeListener propertyChangeListener =
            new ORPropertyChangeListener(sourceFile, document, insightUpdateQueue);
  ...
  public void selectionChanged(@NotNull FileEditorManagerEvent event) {
    ...
    if (newFile != null) {
      ...
      if (FileHelper.isReason(fileType) || FileHelper.isOCaml(fileType)) {
        ...
        InsightUpdateQueue insightUpdateQueue =
            document == null ? null : document.getUserData(INSIGHT_QUEUE);
  ...
  class ORPropertyChangeListener implements PropertyChangeListener, Disposable {
    ...
    ORPropertyChangeListener(
        @NotNull VirtualFile file,
        @NotNull Document document,
        @NotNull InsightUpdateQueue insightUpdateQueue) {
      m_file = file;
      m_document = document;
    ...
    public void propertyChange(@NotNull PropertyChangeEvent evt) {
      if ("modified".equals(evt.getPropertyName())
        ...
        // We invokeLater because compiler needs access to index files and can't do it in the event
        // thread
        ApplicationManager.getApplication()
            .invokeLater(
                () ->
                    m_compiler.runDefault(
                        m_file, () -> m_updateQueue.queue(m_project, m_document)));
        ...
        // PsiFile cmtFile = FileManager.findCmtFileFromSource(m_project,
        // m_file.getNameWithoutExtension());
        // if (cmtFile != null) {
        ...
        if (userData != null) {
          ...
          if (file != null) {
            FileEditor selectedEditor =
                FileEditorManager.getInstance(m_project).getSelectedEditor(file);
            if (selectedEditor instanceof TextEditor) {
              TextEditor editor = (TextEditor) selectedEditor;
              LogicalPosition cursorPosition =
                  editor.getEditor().offsetToLogicalPosition(event.getOffset());
